...
import { appBaseConfig, createInitialRuntimeConfig } from '@/config/AppBaseConfig';
import {
  AppApiConfig,
  BrandingConfig,
  ColorMode,
...
type AppConfigContextValue = {
  ...
  setThemePalette: (mode: ResolvedColorMode, partialTheme: Partial<ThemePalette>) => void;
...
export function AppConfigProvider({ children }: { children: React.ReactNode }) {
  const [config, setConfig] = useState<RuntimeAppConfig>(createInitialRuntimeConfig());
  ...
  const setThemePalette = useCallback((mode: ResolvedColorMode, partialTheme: Partial<ThemePalette>) => {
    setConfig((previousConfig) => ({
      ...previousConfig,
      theme: {
        ...previousConfig.theme,
  ...
  const setBranding = useCallback((partialBranding: Partial<BrandingConfig>) => {
    setConfig((previousConfig) => ({
      ...previousConfig,
      branding: {
